...
fn main() {
    ...
    let application = gtk::Application::new(
        Some("com.github.sheepkill15.osd"),
...
fn build_ui(
    ...
    let val_conv = if let Some(value) = val {
        Some(value.to_string())
    } else {
        None
    };
    let text_conv = if let Some(value) = text {
        Some(value.to_string())
    } else {
        None
    };
    let icon_conv = if let Some(value) = icon {
        Some(value.to_string())
    } else {
        None
    };
    let _overlay = Overlay::new(application, val_conv, text_conv, icon_conv);
}
